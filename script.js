...
const MASTER_DATA = {
    ...
    conference: [
        { en: "Thank you, Mr. Chairman, for the kind introduction.", ja: "座長、丁寧なご紹介ありがとうございます。" },
        { en: "Good morning everyone, my name is [Name], and I am from [University/Institution].", ja: "皆様おはようございます。私は[大学/機関名]の[名前]と申します。" },
        { en: "Today, I would like to present our research on inherently interpretable machine learning models.", ja: "本日は、本質的に解釈可能な機械学習モデルに関する私たちの研究について発表します。" },
        { en: "The title of my presentation is...", ja: "私の発表タイトルは...です。" },
        { en: "My talk today will be divided into five main parts.", ja: "本日の発表は主に5つの部分に分かれています。" },
        { en: "First, I will begin with the background and motivation of our study.", ja: "まず、本研究の背景と動機から始めます。" },
        { en: "Then, I will introduce the problem formulation.", ja: "次に、問題の定式化について紹介します。" },
        { en: "After that, I will explain our proposed methodology.", ja: "その後、提案手法について説明します。" },
        { en: "Next, I will show you the experimental results.", ja: "続いて、実験結果をお見せします。" },
        { en: "Finally, I will conclude my presentation and discuss future work.", ja: "最後に、発表のまとめと今後の課題について述べます。" },
        { en: "Let me start with the background of this research.", ja: "本研究の背景から説明させてください。" },
        { en: "In recent years, artificial intelligence has been widely applied in various fields.", ja: "近年、人工知能は様々な分野で広く応用されています。" },
        { en: "However, the lack of transparency in complex models remains a significant challenge.", ja: "しかし、複雑なモデルにおける透明性の欠如は依然として大きな課題です。" },
        { en: "Many predictive models act as black boxes, making them difficult to trust in high-stakes domains.", ja: "多くの予測モデルはブラックボックスとして機能するため、リスクの高い領域で信頼することが困難です。" },
        { en: "Therefore, Explainable AI, or XAI, has gained considerable attention.", ja: "それゆえ、説明可能なAI（XAI）が大きな注目を集めています。" },
        { en: "While complex models often achieve high accuracy, they sacrifice interpretability.", ja: "複雑なモデルはしばしば高い精度を達成しますが、解釈性を犠牲にします。" },
        { en: "There is a well-known trade-off between predictive accuracy and model interpretability.", ja: "予測精度とモデルの解釈性の間には、よく知られたトレードオフが存在します。" },
        { en: "Previous studies have primarily focused on post-hoc explanation methods.", ja: "先行研究は主に事後的な説明手法に焦点を当ててきました。" },
        { en: "However, these post-hoc explanations can sometimes be unfaithful to the original model.", ja: "しかし、これらの事後的な説明は、元のモデルに対して不誠実（不正確）になることがあります。" },
        { en: "To address this issue, we focus on models that are interpretable by design.", ja: "この問題に対処するため、我々は設計上解釈可能なモデルに焦点を当てています。" },
        { en: "This brings us to the main objective of our study.", ja: "これが本研究の主な目的につながります。" },
        { en: "Our goal is to develop a model that balances both high performance and interpretability.", ja: "私たちの目標は、高いパフォーマンスと解釈性の両方を両立させるモデルを開発することです。" },
        { en: "We formulate this task as a mixed-integer optimization problem.", ja: "我々はこのタスクを混合整数最適化問題として定式化しました。" },
        { en: "Let me explain the proposed framework in detail.", ja: "提案するフレームワークについて詳しく説明させてください。" },
        { en: "As shown in this slide, the architecture consists of two main components.", ja: "このスライドに示すように、アーキテクチャは主に2つの要素で構成されています。" },
        { en: "We employ logistic regression for the baseline probability estimation.", ja: "ベースラインの確率推定にはロジスティック回帰を採用しています。" },
        { en: "We incorporate decision trees to capture non-linear relationships in the data.", ja: "データの非線形な関係を捉えるために、決定木を組み込んでいます。" },
        { en: "This slide shows the objective function of our proposed method.", ja: "このスライドは提案手法の目的関数を示しています。" },
        { en: "The first term represents the loss function, while the second term is the regularization penalty.", ja: "第1項は損失関数を表し、第2項は正則化ペナルティです。" },
        { en: "We introduced a sparsity constraint to make the final rules easier to understand.", ja: "最終的なルールを理解しやすくするために、スパース性制約を導入しました。" },
        { en: "This constraint ensures that only a small number of features are selected.", ja: "この制約により、少数の特徴量のみが選択されることが保証されます。" },
        { en: "To solve this problem efficiently, we utilized a mathematical optimization solver.", ja: "この問題を効率的に解くために、数理最適化ソルバーを利用しました。" },
        { en: "The variables $x_i$ and $y_j$ denote the input features and target labels, respectively.", ja: "変数$x_i$と$y_j$は、それぞれ入力特徴量とターゲットラベルを示します。" },
        { en: "We adopt a piecewise linear approximation to handle the non-linear constraints.", ja: "非線形制約を処理するために、区分線形近似を採用しています。" },
        { en: "Now, let's move on to the experimental setup.", ja: "それでは、実験設定に移りましょう。" },
        { en: "We evaluated our method using several publicly available datasets.", ja: "いくつかの公開データセットを使用して、手法を評価しました。" },
        { en: "The datasets were split into 80% for training and 20% for testing.", ja: "データセットは学習用に80%、テスト用に20%に分割しました。" },
        { en: "We conducted a 5-fold cross-validation to ensure the robustness of our results.", ja: "結果の頑健性を確保するため、5分割交差検証を実施しました。" },
        { en: "All experiments were implemented in Python.", ja: "すべての実験はPythonで実装されました。" },
        { en: "We compared our proposed model against several traditional baselines.", ja: "提案モデルを、いくつかの伝統的なベースライン手法と比較しました。" },
        { en: "The evaluation metrics we used are Area Under the Curve (AUC) and accuracy.", ja: "評価指標として、AUC（曲線下面積）と正解率を使用しました。" },
        { en: "Let's look at the results summarized in Table 1.", ja: "表1にまとめられた結果をご覧ください。" },
        { en: "The horizontal axis represents the number of constraints, and the vertical axis represents accuracy.", ja: "横軸は制約の数を表し、縦軸は精度を表しています。" },
        { en: "The red line indicates our proposed method, while the blue dashed line represents the baseline.", ja: "赤い実線は提案手法を、青い破線はベースラインを表しています。" },
        { en: "As you can see from the graph, our method outperforms the conventional models.", ja: "グラフからお分かりの通り、我々の手法は従来のモデルを上回る性能を示しています。" },
        { en: "Our model achieved an accuracy of 92%, which is highly competitive.", ja: "私たちのモデルは92%の精度を達成し、これは非常に競争力が高いです。" },
        { en: "More importantly, the computational time was significantly reduced.", ja: "さらに重要なことに、計算時間は大幅に短縮されました。" },
        { en: "This indicates that our approach is highly scalable to large datasets.", ja: "これは、我々のアプローチが大規模データセットに対して高い拡張性を持つことを示しています。" },
        { en: "Now I'd like to highlight the interpretability of our model.", ja: "ここで、我々のモデルの解釈性について強調したいと思います。" },
        { en: "Figure 3 illustrates the feature importance extracted from the trained model.", ja: "図3は、学習済みモデルから抽出された特徴量の重要度を示しています。" },
        { en: "This allows domain experts to easily verify the logic behind the predictions.", ja: "これにより、ドメインの専門家は予測の背後にあるロジックを簡単に検証できます。" },
        { en: "Unlike deep neural networks, our model explicitly shows which variables contribute to the output.", ja: "深層ニューラルネットワークとは異なり、私たちのモデルはどの変数が結果に寄与しているかを明示的に示します。" },
        { en: "We also analyzed feature attribution to understand the model's behavior.", ja: "また、モデルの振る舞いを理解するために特徴量帰属の分析も行いました。" },
        { en: "Interestingly, the model assigned higher weights to temporal features.", ja: "興味深いことに、モデルは時間的特徴に対してより高い重みを割り当てました。" },
        { en: "This finding aligns with previous domain knowledge.", ja: "この発見は、これまでのドメイン知識と一致しています。" },
        { en: "However, there are some limitations to our current approach.", ja: "しかし、我々の現在のアプローチにはいくつかの限界があります。" },
        { en: "One limitation is that the mixed-integer optimization can be computationally expensive for extremely large feature spaces.", ja: "一つの限界は、混合整数最適化が極端に巨大な特徴空間において計算コストが高くなる可能性があることです。" },
        { en: "We had to limit the depth of the decision trees to prevent out-of-memory errors.", ja: "メモリ不足エラーを防ぐため、決定木の深さを制限する必要がありました。" },
        { en: "Overcoming this computational bottleneck remains an issue for future work.", ja: "この計算上のボトルネックを克服することは、今後の課題として残されています。" },
        { en: "Let me summarize the main points of my presentation.", ja: "私の発表の要点をまとめさせていただきます。" },
        { en: "In this study, we proposed a novel data-driven approach for interpretable machine learning.", ja: "本研究では、解釈可能な機械学習のための新しいデータ駆動型アプローチを提案しました。" },
        { en: "We successfully formulated the trade-off problem using mixed-integer optimization.", ja: "私たちは混合整数最適化を用いて、トレードオフの問題をうまく定式化しました。" },
        { en: "Our experimental results demonstrated that the proposed method maintains competitive accuracy.", ja: "実験結果は、提案手法が競争力のある精度を維持することを示しました。" },
        { en: "At the same time, it provides clear and transparent decision rules.", ja: "それと同時に、明確で透明性のある決定ルールを提供します。" },
        { en: "For future work, we plan to apply this framework to real-world public policy datasets.", ja: "今後の課題として、このフレームワークを現実の公共政策データセットに適用する予定です。" },
        { en: "We are also considering extending this model to handle multi-class classification tasks.", ja: "また、このモデルを多クラス分類タスクを扱えるよう拡張することも検討しています。" },
        { en: "That brings me to the end of my presentation.", ja: "以上で私の発表を終わります。" },
        { en: "Thank you very much for your kind attention.", ja: "ご清聴ありがとうございました。" },
        { en: "I would be happy to answer any questions you may have.", ja: "ご質問がありましたら喜んでお答えいたします。" },
        { en: "Thank you for your question.", ja: "ご質問ありがとうございます。" },
        { en: "That is a very interesting point.", ja: "それは非常に興味深いご指摘ですね。" },
        { en: "Could you please rephrase your question?", ja: "質問を別の言葉で言い換えていただけますか？" },
        { en: "If I understand correctly, you are asking about the choice of the optimization solver.", ja: "私の理解が正しければ、最適化ソルバーの選択についてのご質問ですね。" },
        { en: "Yes, exactly.", ja: "はい、その通りです。" },
        { en: "We chose this specific method because it guarantees a globally optimal solution.", ja: "大域的最適解を保証するため、我々はこの特定の手法を選択しました。" },
        { en: "To be honest, we haven't considered that perspective yet.", ja: "正直に申し上げますと、その観点についてはまだ検討していません。" },
        { en: "That is a great suggestion for our future research.", ja: "それは私たちの今後の研究にとって素晴らしい提案です。" },
        { en: "In our preliminary experiments, we found that random forests tended to overfit.", ja: "予備実験において、ランダムフォレストは過学習する傾向があることがわかりました。" },
        { en: "The reason why we set the hyperparameter to this value is based on empirical tuning.", ja: "ハイパーパラメータをこの値に設定した理由は、経験的なチューニングに基づいています。" },
        { en: "It depends on the distribution of the underlying data.", ja: "それは基礎となるデータの分布に依存します。" },
        { en: "We plan to investigate that issue in the next phase of our project.", ja: "プロジェクトの次のフェーズでその問題を調査する予定です。" },
        { en: "I'm sorry, I don't have the exact numbers with me right now.", ja: "申し訳ありませんが、今手元に正確な数字を持っていません。" },
        { en: "But I can check the code and get back to you later.", ja: "しかし、コードを確認して後ほどお返事することは可能です。" },
        { en: "As for the scalability, the algorithm runs in polynomial time.", ja: "拡張性に関してですが、このアルゴリズムは多項式時間で動作します。" },
        { en: "Does that answer your question?", ja: "これで質問の答えになっていますでしょうか？" },
        { en: "Thank you, that's very helpful.", ja: "ありがとうございます、大変参考になります。" },
        { en: "Let me go back to slide 15 to explain that again.", ja: "スライド15に戻って、もう一度説明させてください。" },
        { en: "We define interpretability in this context as the sparsity of the active features.", ja: "この文脈における解釈性を、有効な特徴量のスパース性（疎性）と定義しています。" },
        { en: "Compared to traditional logistic regression, our method explicitly handles feature interactions.", ja: "従来のロジスティック回帰と比較して、我々の手法は特徴量の相互作用を明示的に扱います。" },
        { en: "We addressed the non-convexity by reformulating it as a linear programming problem.", ja: "非凸性の問題は、線形計画問題として再定式化することで対処しました。" },
        { en: "The dataset contains several missing values, which we imputed using the mean.", ja: "データセットにはいくつかの欠損値が含まれており、平均値を用いて補完しました。" },
        { en: "This assumption is standard in operations research literature.", ja: "この仮定はオペレーションズ・リサーチの文献では標準的です。" },
        { en: "We normalized the continuous variables to have zero mean and unit variance.", ja: "連続変数は平均がゼロ、分散が1になるように正規化しました。" },
        { en: "The proposed heuristic runs significantly faster than the exact algorithm.", ja: "提案するヒューリスティックは、厳密アルゴリズムよりも大幅に速く動作します。" },
        { en: "There might be a slight degradation in performance when applied to highly noisy data.", ja: "非常にノイズの多いデータに適用した場合、わずかな性能の低下があるかもしれません。" },
        { en: "Our objective was not merely to maximize accuracy, but to provide actionable insights.", ja: "我々の目的は単に精度を最大化することではなく、実行可能な洞察を提供することでした。" },
        { en: "This is crucial for practitioners who need to justify their decisions.", ja: "これは、意思決定を正当化する必要がある実務家にとって極めて重要です。" },
        { en: "Thank you all once again for your valuable feedback.", ja: "貴重なフィードバックをいただき、皆様に改めて感謝申し上げます。" },
        { en: "I look forward to discussing this further during the coffee break.", ja: "コーヒーブレイクの間に、この件についてさらに議論できるのを楽しみにしています。" }
    ]
};
...
const game = {
    ...
    updateUIForLevel() {
        ...
        if (this.currentLevel === 'junior') {
        ...
        } else if (this.currentLevel === 'toeic') {
            ...
            badge.innerText = '国際学会対策';
            badge.className = 'text-[10px] font-bold px-2 py-0.5 rounded-full bg-teal-100 text-teal-600 w-fit mb-1 uppercase tracking-tighter';
        }
    },
